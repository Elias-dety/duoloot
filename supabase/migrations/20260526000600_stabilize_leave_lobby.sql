-- Stabilize leave_lobby RPC.
-- Safe: no table drops and no data deletion besides the authenticated member leaving one lobby.
-- Behavior:
--   member leaves -> removes membership, updates slots/status.
--   owner leaves -> removes owner membership and closes lobby.

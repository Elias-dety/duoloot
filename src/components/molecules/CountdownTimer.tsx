...
export const CountdownTimer: React.FC<CountdownTimerProps> = ({ targetDate, onExpire }) => {
  ...
  if (isExpired) {
    return (
      <div className="flex gap-2 items-center text-red-500 font-bold uppercase tracking-wider text-sm">
        <span>Encerrado</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex gap-4 items-center">
      <div className="flex flex-col items-center">
        ...
        <span className="text-[10px] uppercase text-zinc-500 font-medium">Dias</span>
      </div>
      ...
      <div className="flex flex-col items-center">
        ...
        <span className="text-[10px] uppercase text-zinc-500 font-medium">Horas</span>
      </div>
      <span className="text-zinc-700 font-bold mb-4">:</span>
      <div className="flex flex-col items-center">
        <span className="text-2xl font-bold text-white tabular-nums">{formatUnit(duration.minutes)}</span>
        <span className="text-[10px] uppercase text-zinc-500 font-medium">Min</span>
      </div>
      <span className="text-zinc-700 font-bold mb-4">:</span>
      <div className="flex flex-col items-center">
        <span className="text-2xl font-bold text-orange-500 tabular-nums">{formatUnit(duration.seconds)}</span>
        <span className="text-[10px] uppercase text-zinc-500 font-medium">Seg</span>
      </div>
    </div>
  );
};

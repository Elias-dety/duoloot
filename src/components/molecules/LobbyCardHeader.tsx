    ...
    <div className={`flex items-center gap-3 ${className}`}>
      <div className="relative">
        <Avatar 
          ...
          fallback={owner.name} 
          size="md" 
          className="border-2 border-surface-highlight"
        />
        <div className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-zinc-950 ${
          owner.status === 'online' ? 'bg-success' : owner.status === 'in-game' ? 'bg-info' : 'bg-zinc-600'
        }`} />
      </div>

      <div className="flex flex-col min-w-0">
        <div className="flex items-center gap-2">
          <span className="font-bold text-content-primary truncate">{owner.name}</span>

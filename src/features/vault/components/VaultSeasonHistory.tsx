  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center gap-3 border-b border-[var(--dl-tactical-line)] pb-4">
        <h2 className="font-['Rajdhani'] text-xl font-bold uppercase text-[var(--dl-tactical-text)]">Historico de temporadas</h2>
        <span
          className="dl-hud-label"
          style={{ color: 'var(--dl-tactical-blue)', borderColor: 'rgba(70,183,255,0.34)', background: 'rgba(70,183,255,0.08)' }}
        >
        ...
        {isLoading ? (
          <div className="space-y-3">
            {Array.from({ length: 3 }).map((_, index) => (
              <div key={index} className="animate-pulse border border-[var(--dl-tactical-line)] bg-[var(--dl-tactical-metal)] p-4">
          ...
          <div className="rounded border border-[rgba(255,51,102,0.35)] bg-[rgba(255,51,102,0.08)] p-5 text-center">
            ...
            <p className="font-['Rajdhani'] text-lg font-bold uppercase text-[var(--dl-tactical-red)]">Falha ao carregar temporadas</p>
            <p className="mt-1 text-sm text-white/70">{error}</p>
          </div>
        ) : seasons.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-10 text-center">
            <Archive className="mb-3 h-8 w-8 text-[var(--dl-tactical-blue)]/80" />
            <p className="font-['Rajdhani'] text-lg font-bold uppercase text-[var(--dl-tactical-blue)]">

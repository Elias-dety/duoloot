...
export const LobbyCard: React.FC<LobbyCardProps> = ({
    ...
    Array.from({ length: slotsTotal }).map((_, index) => {
      ...
      return (
        <div
          ...
          className={
            isFilled
              ? 'grid aspect-square w-full place-items-center rounded-full border border-[var(--rank-border)] bg-[radial-gradient(circle_at_50%_35%,var(--rank-bg),rgba(3,8,14,.9)_67%)] text-[var(--rank-primary)] shadow-[0_0_12px_var(--rank-glow)]'
              : 'grid aspect-square w-full place-items-center rounded-full border border-dashed border-[rgba(180,190,205,.42)] bg-white/[0.02] text-lg font-light text-[rgba(245,247,250,.72)]'
          }
          aria-label={isFilled ? 'Jogador no lobby' : 'Vaga aberta'}
        >
          {isFilled ? (
  ...
  // COMPACT CARD
  // ==========================================
  const rankColors = rankTheme.colors;

  const rankPaletteStyle = {
    '--rank-primary': rankColors.primary,
    '--rank-secondary': rankColors.secondary,
    '--rank-accent': rankColors.accent,
    '--rank-border': rankColors.border,
    '--rank-bg': rankColors.background,
    '--rank-glow': rankColors.glow,
    '--rank-text': rankColors.text,
  ...
  const renderCompactCard = () => (
    <DuoFrame
      ...
      style={rankPaletteStyle}
    >
      {/* UI_MARKER: lobby.card.502 | Card de lobby */}
      <article
        data-ui-id={UI_MARKERS.lobby.card.id}
        data-ui-label={UI_MARKERS.lobby.card.label}
        className="dl-panel relative min-w-0 overflow-hidden rounded-[1.35rem] border-0 bg-[radial-gradient(circle_at_15%_0%,var(--rank-bg),transparent_14rem),linear-gradient(135deg,var(--rank-bg),transparent_32%),linear-gradient(180deg,rgba(255,255,255,.04),rgba(255,255,255,.012)),linear-gradient(145deg,#08111d,#050a12_66%,#08111b)] p-3.5 shadow-[0_18px_42px_rgba(0,0,0,.30),0_0_26px_var(--rank-glow)] sm:p-4"
      >
        <UiMarker {...UI_MARKERS.lobby.card} hideInProduction={true} />

        {/* HEADER: status + descrição curta */}
        <header className="relative z-[2] mb-3 grid grid-cols-[auto_minmax(0,1fr)] items-center gap-2.5">
          <div
            className={`inline-flex min-h-[30px] shrink-0 items-center justify-center gap-1.5 rounded-full border px-2.5 text-[0.62rem] font-black uppercase tracking-[0.08em] ${statusClassName}`}
          >
          ...
          <div className="min-w-0 overflow-hidden">
            <p className="m-0 overflow-hidden whitespace-nowrap font-['Inter'] text-[0.72rem] font-semibold leading-snug text-[rgba(235,239,246,.58)] [mask-image:linear-gradient(90deg,#000_0%,#000_78%,transparent_100%)] [-webkit-mask-image:linear-gradient(90deg,#000_0%,#000_78%,transparent_100%)]">
              {shortDescription}
            </p>
          </div>
        </header>

        {/* IDENTIDADE: avatar + nome + role/agente */}
        <section className="relative z-[2] grid grid-cols-[62px_minmax(0,1fr)] items-center gap-3 border-b border-[var(--rank-border)] pb-3.5">
          <div className="relative grid h-[62px] w-[62px] shrink-0 place-items-center overflow-visible rounded-[1rem] border border-[var(--rank-border)] bg-[linear-gradient(145deg,var(--rank-bg),rgba(3,8,15,.96))] shadow-[0_0_20px_var(--rank-glow),inset_0_0_0_1px_var(--rank-border)]">
            {avatarRankIconSrc ? (
              <img
                src={avatarRankIconSrc}
                alt={`Elo ${rankTheme.label}`}
                className="relative z-[2] h-[58px] w-[58px] scale-[1.12] object-contain drop-shadow-[0_0_14px_var(--rank-glow)]"
                loading="lazy"
                onError={handleRankIconError}
              />
            ) : (
              <span className="font-['Rajdhani'] text-xl font-black text-white">?</span>
            )}
          </div>

          <div className="min-w-0">
            <h3 className="m-0 max-w-full overflow-hidden text-ellipsis whitespace-nowrap font-['Rajdhani'] text-[1.6rem] font-black uppercase leading-[.9] tracking-[.02em] text-white sm:text-[1.75rem]">
              {ownerName}
            </h3>
            ...
            <div className="mt-1.5 flex min-w-0 items-center gap-1.5 overflow-hidden text-[0.76rem] font-black sm:text-[0.8rem]">
              ...
              <span className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-[var(--rank-accent)]">
                {mainAgent}
              </span>
            </div>
          </div>
        ...
        <section className="relative z-[2] border-b border-[rgba(160,180,205,.12)] py-3.5">
          <div className="mb-2.5 flex items-center justify-between gap-3">
            <div className="inline-flex items-center gap-2 rounded-xl border border-[var(--rank-border)] bg-[var(--rank-bg)] px-2.5 py-2 shadow-[0_0_14px_var(--rank-glow)]">
              ...
              <span className="whitespace-nowrap font-['Rajdhani'] text-[1.35rem] font-black leading-none text-white">
        ...
        {/* MATCHMAKING: barra + porcentagem */}
        <section className="relative z-[2] border-b border-[rgba(160,180,205,.12)] py-3.5">
          <div className="mb-2 flex items-center justify-between gap-3">
            <span className="min-w-0 overflow-hidden text-ellipsis whitespace-nowrap text-[0.78rem] font-black uppercase tracking-[.08em] text-[#16d7ff]">
              Matchmaking
            </span>

            <span className="whitespace-nowrap font-['Rajdhani'] text-[1.65rem] font-black leading-none text-[#ffd166]">
              {matchPercent}%
            </span>
          </div>
          ...
          <div className="h-2.5 rounded-full bg-[rgba(160,180,205,.13)] p-[2px] shadow-[inset_0_0_0_1px_rgba(160,180,205,.08)]">
            <div
              className="h-full rounded-full bg-[linear-gradient(90deg,#14d8ff_0%,#20ddca_42%,#ffd166_100%)] shadow-[0_0_14px_rgba(20,216,255,.16)]"
        ...
        {/* BOTÕES */}
        <footer className="relative z-[2] grid grid-cols-2 gap-2.5 pt-3.5">
          <button
            type="button"
            className="inline-flex min-h-[42px] min-w-0 items-center justify-center rounded-xl border border-[rgba(160,180,205,.17)] bg-[linear-gradient(180deg,rgba(255,255,255,.06),rgba(255,255,255,.02))] px-3 text-[0.78rem] font-black uppercase tracking-[.08em] text-[rgba(245,247,250,.80)] transition hover:-translate-y-0.5 hover:bg-white/[.07]"
          ...
          <button
            type="button"
            className={`inline-flex min-h-[42px] min-w-0 items-center justify-center rounded-xl px-3 text-[0.78rem] font-black uppercase tracking-[.08em] text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-55 ${
              actionDisabled
  ...
  // ==========================================
  // FEATURED CARD (card grande original)
  // ==========================================
  ...
  const renderDetailsModal = () => (
    <>
      {/* UI_MARKER: lobby.details-modal.504 | Modal do lobby */}
      <div
        data-ui-id={UI_MARKERS.lobby.detailsModal.id}
        data-ui-label={UI_MARKERS.lobby.detailsModal.label}
        className="fixed inset-0 z-50 grid place-items-center bg-black/65 p-5 backdrop-blur-xl"
        onClick={() => setIsDetailsOpen(false)}
      >
        <UiMarker {...UI_MARKERS.lobby.detailsModal} hideInProduction={true} />
      <div
  ...
  return (
    <>
      {density === 'featured' ? renderFeaturedCard() : renderCompactCard()}
      {isDetailsOpen ? renderDetailsModal() : null}
    </>
  );
};
